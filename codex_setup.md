...
**Scope:** Studio Prime targets **web products** — its terminal success condition is an HTTP-200 live URL (deployed or localhost). Non-web targets (mobile, desktop, CLI/library, data pipelines) are detected in Phase 1 and either run under a substituted per-class liveness condition or checkpoint-exit with `[UNSUPPORTED_TARGET: non-web]` rather than being forced into a web shape.
...
### Cost profile — before you walk away
An unattended Studio Prime run fans out web research across 6 phases, a ≤4-worker build swarm, and an adversarial sub-agent review at every gate, plus up to two northstar re-walks — a non-trivial PRD is many millions of tokens. On a metered API a single overnight run can be expensive. **Set a provider-side spend cap (Anthropic/OpenAI/Google all support hard usage limits) BEFORE an unattended run.** Optionally set `STUDIO_MAX_WALLCLOCK_HRS` and/or `STUDIO_MAX_TOOL_CALLS` in the environment — Studio Prime tracks wall-clock + tool-call count in `.studio/state/budget.md` and checkpoint-exits on breach. (These are observable proxies; Studio Prime does not claim a token/$ cap the engine cannot observe.)

### Localhost survival (no-creds runs)
On the no-credentials path Studio Prime leaves a localhost server running for handoff — but only after a **detach-survival probe** proves the process actually outlives the agent session on THIS host (it prefers daemon-owned supervision: `docker compose up -d`, `systemd --user`, `pm2`, detached `tmux`/`screen`, or a Windows Scheduled Task). If the probe fails (e.g. a bare `Start-Process` child that dies with the job object), Studio Prime does NOT claim `[LOCAL_LIVE]` — it emits `[DEPLOY_READY: localhost cannot outlive this session on <host>]` and hands you the exact `deploy_ready.sh` go-live commands instead. HANDOFF.md records a post-session liveness assertion you can run after the agent exits.
...
*   **Proof-of-Work Anti-Hallucination:** LLMs inherently struggle to differentiate between intended outcomes and actual execution results. To counteract this, Codex Prime enforces a strict, three-part empirical verification protocol. Before asserting task completion, the agent must generate a `<prediction>` of the expected terminal output, execute the command via the `shell` tool, and synthesize a `<divergence_analysis>`. Critically, every gate command is run with its output captured to disk (`tee` to `.studio/state/pow/*.log`) carrying an unguessable RC/timestamp liveness nonce tied to that exact invocation, and the `<stdout>` field is populated by RE-READING that on-disk log — never typed from memory. A proof-of-work block with no corresponding `.log` file on disk is auto-classified `[UNVERIFIED]` and blocks the gate. The Apex reviewer (a clean, isolated context) takes the on-disk log path as its only evidence and asserts on the RC/nonce lines, so the writer of a claim and its grader are never the same context.
...
*   **Parallel Build Swarm (P3/P4):** The build interior of Phase 3 (scaffolding) and Phase 4 (implementation) is parallelized across an ownership-partitioned swarm of `spawn_agents_on_csv` workers — each owns a disjoint set of files, runs its own module-scoped proof-of-work, and reports back via `report_agent_job_result`. The main agent is the sole `apply_patch` merge authority and RE-RUNS the full phase proof-of-work against the merged whole before the (always single-threaded) Apex gate — speed comes from parallel construction, never from trusting unverified parallel claims. With no `spawn_agents_on_csv` available the swarm degrades cleanly to sequential execution (same gates, same correctness).
*   **Proactive Context Engineering:** Rather than waiting for Codex's silent auto-compaction at `context - 13k tokens`, the agent actively manages context pressure across GREEN/AMBER/RED budget tiers, running a compaction ladder (archive completed `update_plan` items → summarize stale decisions → offload context-heavy subtasks to a sub-agent → write `context_checkpoint.md`) so every decision lands in `.studio/` BEFORE the window flushes. The host compaction path here is Codex's native auto-compact (`context - 13k`) plus optional `/compact` — and because `update_plan` does not persist across responses, `.studio/todos.md` is always the canonical task list.
...
5.  **Phase 5: Stylize (UI/UX Polish)**
    *   **Function:** Transforms functional interfaces into production-grade aesthetics. As its mandatory first step, accelerates by default by pulling a curated design-skill matched to the project archetype — enumerated NON-INTERACTIVELY by fetching the registry index (`curl …/skills/index.json`, never the interactive `typeui.sh list` which hangs headless) and pulling the chosen slug's SKILL.md directly; any CLI use is the fully-flagged `pull <slug> -f skill -p codex --dry-run`. Registry skills routinely ship banned hex and zero OKLCH, so Studio Prime expects to TRANSFORM (hex→OKLCH conversion + black/white clamp), not adopt — reconciling the usable structure/typography/spacing into `design-system/MASTER.md`, with Studio Prime's banned-pattern rules always winning; only genuine unavailability degrades gracefully to the built-in standard. Enforces the 2026 Impeccable Design Standard, ensuring **WCAG 2.1 AA accessibility verified via an executed runtime axe-core/pa11y audit against the running app (critical/serious violations are a BLOCKER, and a stub-only or audit-only submission is itself a BLOCKER)**, comprehensive component state matrices, fluid motion physics, and LCP (<2.5s) / CLS (<0.1) metrics.
6.  **Phase 6: Release**
    *   **Function:** The final pre-flight operational check. Tests **graceful OS signal shutdowns (`SIGTERM`/`SIGINT`)** against a DISPOSABLE instance (the drain handler is verified, then the product is (re)started — the kill never leaves the app dark at handoff), runs production **DB migrations**, verifies the **synthetic alert pipeline**, and — when deploy credentials were provided at intake (the credentials ARE the authorization) — **autonomously deploys LIVE and runs post-deploy smoke tests against the production URL**; when no credentials were provided, it starts the production server locally as a DETACHED process that survives the session and runs the smoke suite against `http://localhost:<port>`. A final liveness re-probe asserts the URL returns HTTP 200 immediately before sign-off. Concludes with a comprehensive 17-section `HANDOFF.md` consolidation (recording the live URL or the running localhost URL + PID), the final **Northstar Validation Gate**, and a concise **Deployment Briefing** emitted at sign-off — the verified live status, deploy-target operations (redeploy/rollback/logs, or the exact go-live steps + ranked host recommendation on no-creds runs), proactive next-step suggestions, and the Codex resume command.
...
│   ├── context_checkpoint.md # Heartbeat context handoff (minimal-loss, re-derivable resume)
│   ├── effects.md          # Idempotency ledger — guards re-runs/resumes from re-applying side effects
