...
Codex Prime is not just a prompt; it is a **Production-Grade AI Operating System**. It transforms your standard AI coding assistant into a relentless, autonomous Principal Architect, UI/UX Designer, and Zero-Trust Red Teamer.
...
With Codex Prime, you achieve the ultimate "Sleep Test": Start a session, go to sleep, and wake up to a completed, rigorously audited application. **Codex CLI passes this test natively because `codex exec` is engineered from day one for unattended runs (CI, cron, pre-merge hooks) — no human in the loop required.**
...
*   **Round 3 (Synthesis):** A principal engineer persona adjudicates the debate, classifying findings into `[BLOCKER]` (halts execution, mandates a safe `git stash` rollback, and triggers HaaS) or `[TECH_DEBT]` (logs to `.studio/todos.md` but permits progression); absence of either yields the overall verdict `[GREEN_FLAG]` (clean pass).
...
6.  **Phase 6: Release**
    *   **Function:** The final pre-flight operational check. Integrates **graceful OS signal shutdowns (`SIGTERM`/`SIGINT`)**, production **DB migrations**, **synthetic alert pipeline verification**, and executes **automated post-deploy smoke tests** against production, concluding with a comprehensive 17-section `HANDOFF.md` consolidation and the final **Northstar Validation Gate**.
...
*   **Human-as-a-Service (HaaS) Feedback:** Explicit approvals, architectural overrides, or credential provisions when the system halts at an enforced gate, delivered through Codex's `request_user_input` tool.
