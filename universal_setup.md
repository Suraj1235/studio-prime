...
[← Back to README](README.md) · Setup guide for [`studio_prime_v5.5.md`](studio_prime_v5.5.md)

**The omni-executioner prime with runtime platform auto-detection — one prompt, six platforms.**

**V5.5 — covering OpenCode, Claude Code, OpenClaw, Codex CLI, Antigravity, and Cursor (degraded fallback).**

> **The Sleep Test:** hand the agent a PRD + your API keys, walk away, and come back to a finished, tested, documented, **deployable** product. Every prime enforces an *Autonomous Execution Contract* so it never stalls, loops, or silently gives up unattended — and a production-grade *proof-of-work* gate (migrations, Docker/CI-CD, OWASP + secrets scanning, executed E2E/smoke tests, structured logging, health probes, graceful shutdown, WCAG 2.1 AA, a 17-section `HANDOFF.md`) so what comes back is real, not hallucinated.
...
1. **Pick your host** and copy its prime into the host's system-prompt/instructions location:
   ...
   | Cursor / unknown | `studio_prime_v5.5.md` | host's system-prompt area |
...
### What's New in V5.5 (Codex CLI + Antigravity)

V5.5 extends Studio Prime to two new platforms with fundamentally different agent paradigms:
...
The Platform Auto-Detection block now probes 5 sub-agent dispatchers and 5 shell-tool variants, with explicit derivation rules for Codex CLI and Antigravity. The platform-specific prime set expanded from 4 (OpenCode, Claude Code, OpenClaw, plus the universal V5.4) to 6 (added Codex CLI and Antigravity as native variants; universal V5.5 carries 6-platform auto-detection covering all five native hosts plus Cursor as a degraded fallback). The degradation logic now distinguishes between dispatcher-style sub-agents (OpenCode `Task`, Claude `Agent`, OpenClaw `sessions_spawn`, Codex `spawn_agents_on_csv`) and native isolated subagent spawn (Antigravity `invoke_subagent` — async, clean-context, optional Git-worktree isolation; `browser_subagent` is the browser-scoped variant invoked via `/browser`, with parallel multi-agent orchestration available through the Agent Manager).

### What's New in V5.4

V5.4 is a substantial leap forward in autonomy, safety, and host-agnosticism. Key additions:
...
6.  **Phase 6: Release** — The final pre-flight operational check. Integrates **TESTED graceful OS signal shutdown (timed `SIGTERM` drain ≤35s)**, production **DB migrations (dry-run gated)**, **synthetic-error → alert-propagation verification**, a **timed rollback dry-run (<5min)**, and executes **automated Playwright/Cypress smoke tests** against the deployed environment (5xx → autonomous safe rollback + HaaS notify), concluding with a comprehensive 17-section `HANDOFF.md` consolidation (validated placeholder-free) and the final **Northstar Validation Gate**.
...
Studio Prime is fully autonomous. The human is treated as a blocking API, invoked only when the agent genuinely cannot proceed alone. HaaS triggers fall into four categories:

1.  **Destructive & Network Gates** — The following commands ALWAYS require human authorization: `rm -rf`, `npm publish`, database drops, `force-push`, `chmod 777`, any `curl`/`wget`/`nc`/`netcat` sending data to external hosts, any `curl|wget` piping to `sh`/`bash`/`python`, and port-scanning tools like `nmap`/`masscan`. Legitimate API calls are allowed without authorization, but suspicious patterns are flagged.
2.  **Credential Gaps** — API keys, OAuth tokens, or external service accounts that the agent cannot auto-generate trigger an immediate HaaS request with a clear list of what is needed and why.
...
*   **Human-as-a-Service (HaaS) Feedback:** Explicit approvals, architectural overrides, or credential provisions when the system halts at an enforced gate (destructive command detection, Red Team blocker, repair-limit exhaustion, PRD conflict).
