...
While the Studio Prime architecture runs on multiple platforms, **Claude Code is the most rigorously instrumented environment for it.** Claude Code natively supports true sub-agent dispatch via the `Task` tool with the `subagent_type` parameter, deterministic system-side hooks via `.claude/settings.json` (hooks are merged into the existing `.claude/settings.json` under a top-level `hooks` key), and structured user prompts via the `AskUserQuestion` tool. When Studio Prime launches its Apex Red Team to audit code, Claude Code physically spins up a fresh, blinded context window (same model weights — this reliably catches forgetting/drift defects, though it is not a structurally independent judge of the implementer's own reasoning). The Red Team is fed only the northstar, this-phase artifacts, and the on-disk proof-of-work logs — not the developer's rationale — and grades from that objective evidence. This delivers **adversarial review free from the conversational context contamination that plagues in-thread self-critique**; when dispatch is unavailable the reviewer re-EXECUTES the gate commands (rather than re-reading prior prose) and the verdict is flagged `[UNVERIFIED-REVIEW]`.
...
> **Localhost survival (no-creds path):** whether a detached localhost server outlives the agent session depends on your host. On some sandboxes/job-objects the process tree is torn down at session exit. Studio Prime now runs a **detach-survival probe** before claiming `[LOCAL_LIVE]` and prefers daemon-owned supervision (`docker compose up -d`, `pm2`, a `systemd --user` unit, or a Windows Scheduled Task) where available. If the probe fails on your host, it will tell you plainly (`[DEPLOY_READY: localhost cannot outlive this session]`) and hand you `deploy_ready.sh` to start it yourself. The probe outcome per host is noted in the Deployment Briefing.

> **Cost profile — set a spend cap before you walk away:** an unattended run fans out research across 6 phases, a parallel build swarm, and an adversarial review at every gate, so a non-trivial PRD can consume many millions of tokens. Set a **provider-side hard spend cap** (Anthropic supports usage limits) BEFORE an overnight run — the engine cannot observe a $/token ceiling. Optionally set `STUDIO_MAX_WALLCLOCK_HRS` and/or `STUDIO_MAX_TOOL_CALLS`: these observable proxies are tracked in `.studio/state/budget.md` and trigger a clean checkpoint-exit on breach.
...
*   **Proof-of-Work Anti-Hallucination:** LLMs inherently struggle to differentiate between intended outcomes and actual execution results. To counteract this, Claude Code Prime enforces a strict, three-part empirical verification protocol. Before asserting task completion, the agent must generate a `<prediction>` of the expected terminal output, execute the command via the `Bash` tool **capturing stdout to an on-disk log (`.studio/state/pow/*.log`) with a shell-written `RC`/`NONCE` liveness line**, then populate the `<divergence_analysis>` by RE-READING that log — never by typing remembered output. A proof-of-work block with no corresponding on-disk log is auto-classified `[UNVERIFIED]` and blocks the gate, and the Apex reviewer (a separate, clean context) grades from the on-disk log alone — so the writer and the grader are never the same context.
...
*   **Requirement Documents (PRDs):** Structured text outlining the core problem, target audience, and functional requirements. **Scope:** Studio Prime targets **web products** — its terminal liveness condition (an HTTP-200 URL or localhost server) is web-shaped by construction. For a non-web target (mobile, desktop, CLI/library, data pipeline) the agent detects the target class in Phase 1 and either substitutes an appropriate live end-state (build artifact + simulator/binary smoke) or checkpoint-exits with `[UNSUPPORTED_TARGET: non-web]` rather than silently forcing a web shape. For vague PRDs the agent records a separated `[ASSUMPTION]` register and reports `VALIDATED-WITH-ASSUMPTIONS` so self-filled gaps are surfaced, not laundered into a clean pass.
...
│   ├── context_checkpoint.md    # Heartbeat checkpoint — minimal-loss, re-derivable resume across /compact or crash
...
│   └── deployment_briefing.md   # Final deployment briefing (also in chat + HANDOFF §10)
...
The agent itself cannot invoke slash commands programmatically; the `/compact` command must be issued by the human operator (this is the documented partial-Sleep-Test caveat). Context engineering is therefore **proactive**: the agent tracks its own context budget across GREEN / AMBER / RED tiers and runs a compaction ladder — archiving completed todos, collapsing old decisions, and offloading context-heavy reads to a sub-agent ("read big, return small") — before pressure becomes critical. Claude Code Prime mitigates the no-auto-compact limitation in two ways:
1.  **Proactive prompting + checkpointing:** At AMBER (~15+ tool calls or any file >500 lines read) the agent runs the lighter ladder steps; at RED it writes a `context_checkpoint.md` for a minimal-loss, re-derivable resume and emits the `[CONTEXT NOTICE]` recommending `/compact` — informational, never blocking; it keeps working in the same response.
2.  **Prime Directive #2 (Memory Persistence) as the safety net:** Every architectural decision, blocker, and phase verdict is written to `.studio/` immediately, and `context_checkpoint.md` is rewritten on a heartbeat (after every phase and before every sub-agent dispatch), so a crash at any tier has a recent anchor. On resume the agent reads `context_checkpoint.md` FIRST (resuming from its `next_concrete_action`), then `architecture/decisions.md`, `.studio/todos.md`, and `.studio/state/northstar.md`. This is a **minimal-loss, re-derivable resume** — the `.studio/` markdown tree travels across hosts, but native task stores are host-local and are rebuilt from `.studio/todos.md` on a cross-host resume. A side-effect ledger (`.studio/state/effects.md`) makes re-runs idempotent: deploys, migrations, and synthetic-error injections already applied this run are skipped or guarded rather than blindly repeated. The filesystem is the source of truth, not the conversation.
