...
studio_prime_v5.5.md      # Universal prime — auto-detects the host & degrades gracefully
...
1. **The Sleep Test.** Given a PRD + the required API keys/resources, a user triggers the agent, walks away, and returns to a finished, **deployable** product. A prime must carry the agent intake → 6-phase lifecycle → deploy with **no stall, infinite loop, silent give-up, or human-wait that lacks an unattended fallback**. See the *Autonomous Execution Contract* in each prime.
